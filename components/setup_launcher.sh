# Phase 3, step 3 — launcher setup. Writes the config the launcher snippet
# reads, appends that snippet idempotently to both .bashrc and .zshrc (safe
# to re-run), and installs archkill as a real $PREFIX/bin script so it
...
# both rc files — Termux's actual default shell is bash (.bashrc), zsh
# only lives inside the Arch container, never in Termux itself.
...
phase3_setup_launcher_run() {
  ...
  if [ "${TDE_DRY_RUN:-0}" = "1" ]; then
    log_info "[dry-run] would write launcher config, append idempotent snippet to .bashrc and .zshrc, install archkill to \$PREFIX/bin"
    return 0
  fi
